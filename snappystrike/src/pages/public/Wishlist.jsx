      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-border pb-4 gap-4">
          <h1 className="text-2xl md:text-3xl font-extrabold flex items-center gap-3">
            <Heart className="text-destructive" size={28} fill="currentColor" /> Your Wishlist
          </h1>
          <div className="flex flex-wrap items-center gap-3">
            {!user && (
               <Link to="/login" className="text-xs md:text-sm font-medium text-muted-foreground hover:text-primary transition-colors">
                 Log in to sync &rarr;
               </Link>
            )}
            <Link to="/" className="text-xs md:text-sm font-medium bg-primary text-primary-foreground px-4 py-2 rounded-full hover:opacity-90 transition-opacity">
              ← Explore
            </Link>
          </div>
        </div>
